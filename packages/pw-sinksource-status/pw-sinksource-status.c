/*
 * pw-sinksource-status - PipeWire audio status tool
 *
 ...
 *
 * Usage: pw-sinksource-status [--i3statusrs] [--debug] <sink|source>
 */
...
/*
 * Config format (~/.config/pw-sinksource-status/config):
 *   [sink]
...
static void load_config(struct state *s) {
  ...
  char path[512];
  snprintf(path, sizeof(path), "%s/.config/pw-sinksource-status/config", home);
